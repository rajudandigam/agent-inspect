...
A **v1.9 LangGraph-shaped callback workflow** using `@agent-inspect/langchain`.

The recipe calls the LangChain callback adapter with deterministic LangGraph-style metadata. It does not import or run LangGraph, LangSmith, a model provider, hosted tracing, API keys, or network requests.
...
- The adapter preserves graph, node, task, branch, checkpoint, handoff, and thread metadata under bounded `langGraph` attributes.
...
- There is no separate `@agent-inspect/langgraph` package in v1.9; LangGraph-shaped data is supported through `@agent-inspect/langchain` first.
