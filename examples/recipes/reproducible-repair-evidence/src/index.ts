...
const taskTxt = `# Synthetic repair task

Restore platform-aware launcher selection without swallowing launcher failures.

Do not apply the candidate patch until the hidden oracle result is reviewed.
`;

const reproductionJson = `${JSON.stringify(
  {
    reproductionEnvelopeVersion: "0.1",
    runId,
    target: {
    ...
    environment: {
      ...
      allowlistedVariableNames: ["CI"],
      modelFingerprint: null,
    },
    task: {
      taskFile: "task.txt",
      taskFileSha256: sha256Hex(taskTxt),
    },
    validation: {
      command: "pnpm",
...
const oracleResultJson = `${JSON.stringify(
  {
    ...
    checks: [
      ...
      { id: "error-propagation", passed: false },
    ],
  },
  null,
  2,
...
const files: EvidencePackagedFile[] = [
  ...
  { path: "task.txt", content: taskTxt, role: "other" },
...
const manifest = buildEvidenceManifest({
  ...
  sourceHashes: [{ runId, algorithm: "sha256", hash: sha256Hex(failedTrace) }],
  redactionProfile: "share",
  assessmentStatus: "SAFE WITH WARNINGS",
  note:
    "Synthetic caller-owned repair records only. Manifest assessment is not a share certification; review before attaching to support.",
  files,
...
process.stdout.write(
  [
    ...
    `Evidence directory: ${relativeEvidenceDir}`,
  ].join("\n") + "\n",
);
