...
- `share` and `strict` rewrite http(s) URLs rather than dropping them: userinfo and credential-looking query or fragment params are replaced, identifier-like path segments become `[id]`, and scheme, host and path stay, so a retry on the same page is still recognizable. Only http and https URLs are rewritten: credentials in other schemes such as `postgres://user:pass@host/db` are left as they are, so check those by hand.
